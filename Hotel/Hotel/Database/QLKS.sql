...
select A.MAPHG, MALOAIPHG, DONDEP, TRANGTHAI,TANG, GHICHU, CTPHG.MAHD, CHECKEDIN, NGNHANPHG, NGTRPHG    
                          ...
                          left join CTPHG on A.MAPHG = CTPHG.MAPHG    
                          left join HOADON on CTPHG.MAHD = HOADON.MAHD   
						  where (HOADON.MAHD is null) OR EXISTS(
							SELECT MAHD
							FROM CTPHG
							WHERE CTPHG.MAPHG = A.MAPHG AND (NGNHANPHG > '2023-12-03 14:00:00.000' NGTRPHG < '2023-12-03 12:00:00.000')
							)
SELECT p.MAPHG
FROM PHONG p
WHERE NOT EXISTS (
  SELECT 1 FROM CTPHG c
  WHERE p.MAPHG = c.MAPHG
  AND c.NGNHANPHG <= '2023-12-03'
  AND c.NGTRPHG >= '2023-12-04'
)
select distinct p.MAPHG , MALOAIPHG from PHONG p left join CTPHG c on p.MAPHG = c.MAPHG where c.MAPHG is null or c.NGNHANPHG > '2023-12-19 08:30:52' or c.NGTRPHG < '2023-12-19 08:30:52'
INSERT INTO HOADON(MAKH, MANV) VALUES ('KH06', 'NV01')
select distinct p.MAPHG , MALOAIPHG from PHONG p left join CTPHG c on p.MAPHG = c.MAPHG where c.MAPHG is null or c.NGNHANPHG > '12/12/2023 09:03:55 AM' or c.NGTRPHG < '12/12/2023 09:03:55 AM'
select * from PHONG
 select A.MAPHG, MALOAIPHG, DONDEP, TRANGTHAI, TANG, GHICHU, CTPHG.MAHD, CHECKEDIN, CHECKEDOUT, NGNHANPHG, NGTRPHG    
                          from PHONG A    
                          left join CTPHG on A.MAPHG = CTPHG.MAPHG    
                          left join HOADON on CTPHG.MAHD = HOADON.MAHD 
SELECT * FROM HOADON
DELETE FROM HOADON WHERE MAHD = 'HD0001' OR MAHD = 'HD00017'
update HOADON set CHECKEDIN = 0 from HOADON inner join CTPHG on HOADON.MAHD = CTPHG.MAHD where CHECKEDIN is null
