...
namespace Hotel.Test.SourceCode___Lam_theo_nay_ne
{
    ...
    public class UC_EmployeeManagementTests
    {
        ...
        public void TbSearch_TextChanged_ShouldFilterEmployeeData()
        {
            ...
            string query =
                "SELECT MANV as 'Mã Nhân Viên', NHOTEN as 'Họ Tên', NCCCD as 'CCCD', NGIOITINH as 'Giới Tính', CONVERT(DATE, NNGSINH) as 'Ngày Sinh', NSDT as 'SDT', NDIACHI as 'Địa Chỉ', NEMAIL as'Email', CHUCVU as 'Chức Vụ', cast(LUONG as decimal) as 'Lương' " +
                "from NHANVIEN " +
            ...
            dataTable.Rows.Add("NV001", "John Doe", "123456789", "Nam", DateTime.Now.AddYears(-30).ToShortDateString(),
                "0123456789", "123 Street", "john.doe@example.com", "Nhân viên", 5000000);
            ...
            Assert.That(dataGridView.Rows[0].Cells["Họ Tên"].Value, Is.EqualTo("John Doe"),
                "The filtered row should match the search text.");
        }
        ...
        //     var changePasswordButton = cpDialog.Controls.OfType<Guna2Button>()
        //         .FirstOrDefault(btn => btn.Name == "guna2Button1");
        ...
        //         It.Is<string>(q =>
        //             q.Contains("UPDATE TAIKHOAN SET MATKHAU = '" + newPassword + "' WHERE MANV = 'NV001'")),
        ...
        // {
        //     // Arrange
        //     var guna2Button1 = (Guna2Button)typeof(UC_Employee2)
        //         .GetField("guna2Button1", BindingFlags.NonPublic | BindingFlags.Instance)
        //         .GetValue(_ucEmployee2);
        //     var saveFileDialog = (SaveFileDialog)typeof(UC_Employee2)
        //         .GetField("saveFileDialog1", BindingFlags.NonPublic | BindingFlags.Instance)
        //         .GetValue(_ucEmployee2);
        ...
        //         fn => fn.ToExcel(It.IsAny<DataGridView>(), It.Is<string>(s => s.EndsWith("output.xlsx"))), Times.Once);
        // }
    }
}